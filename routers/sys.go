	...
	go func() {
		...
		for {
			...
			case <-ticker.C:
				...
				pusherData = append(pusherData, CountData{Time: now, Total: 0})
				playerData = append(playerData, CountData{Time: now, Total: 0})
...
func (h *APIHandler) GetServerInfo(c *gin.Context) {
	c.IndentedJSON(http.StatusOK, gin.H{
		...
		"Server":           fmt.Sprintf("%s/%s,%s (Platform/%s;)", "柏晓技术", BuildDateTime, BuildVersion, strings.Title(runtime.GOOS)),
		"memData":          memData,
		"cpuData":          cpuData,
